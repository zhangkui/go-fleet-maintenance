...
## Docker Build Template
    ...
    ./build_benzhi_docker.sh go-fleet-maintenance-bug009-candidate linux/amd64

Docker 命令仅作为评测模板保留。本题未运行 Docker 或架构验证，BENZHI_VALIDATION 中均明确记录为 NOT_RUN。

## BUG-009 Verification Command

    cd '/app' && GOTOOLCHAIN=local go test ./internal/service -count=1 -run '^TestBug009_VehicleLicenseExpiry$'

诊断分支保持缺陷代码，预期该命令退出码为 1。
